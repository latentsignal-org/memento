...
function NewProjectContent() {
    ...
    const handleSend = useCallback(
        (message: string) => {
            if (!draftId) return;
            // Optimistically append the user turn so the chat doesn't jump-cut.
    ...
    const handleBundleChange = useCallback(
        async (next: EntityBundle) => {
            if (!draftId) return;
            // Debounce by skipping while we're typing in the name input is overkill
            // for the hackathon — a PATCH per keystroke is fine on localhost.
            try {
                ...
                <header className="flex flex-wrap items-end justify-between gap-4">
                  ...
                  Collector agent
                            ...
                            <p className="max-w-[760px] text-ui-medium text-on-surface-variant">
                                Build a source-backed project draft from your archive, then
                                curate the people, messages, and threads that belong in it.
                            </p>
                        </div>
                    </div>
                </header>
                ...
                <div className="grid min-h-[560px] grid-cols-1 gap-5 lg:min-h-0 lg:flex-1 lg:grid-cols-12">
                    ...
                    <div className="h-[60dvh] min-h-[380px] lg:h-auto lg:min-h-0 lg:col-span-5">
                        <AgentChat
                            ...
                            placeholder={
                                draftId
                                    ? "e.g. 'Let's start a project about the kitchen remodel.'"
                                    : "Setting up draft…"
                            }
                            enableMentions
                        />
