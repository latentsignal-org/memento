...
import {computeCompletedSteps, CONCEPT_STEPS_CONFIG} from "@/lib/agent-steps";

const SIMULATION_UI_ENABLED = process.env.NEXT_PUBLIC_MEMENTO_AGENT_SIMULATION === "1";

const CONCEPT_STEPS = CONCEPT_STEPS_CONFIG.steps;

interface ConceptCompileButtonProps {
    slug: string;
...
export default function ConceptCompileButton({
                                                 slug,
                                                 hasGenerated = true,
                                                 onRunningChange,
                                                 cardLayout = "stack",
                                                 ...
                                                 simulationDelayMs,
                                             }: ConceptCompileButtonProps) {
    const router = useRouter();
    const [open, setOpen] = useState(false);
    const resumeStartedRef = useRef(false);
    const {runIdFromURL, rememberRun, clearRun} = useAgentRunURLState("concept_compile");
    const {events, isRunning, error, run, resume, reset} = useAgentStream((e) => {
        if (e.type === "done") {
            void revalidateEntityPath(`/concepts/${slug}`)
    ...
    const completedSteps = useMemo(
        () => computeCompletedSteps(CONCEPT_STEPS_CONFIG, events),
        [events],
    );
    ...
    const currentActivity = useMemo(() => {
        ...
        if (!latest) return "Preparing concept agent";
    ...
    const start = async (simulate = false) => {
        ...
        const endpoint = shouldSimulate
            ? `/api/concepts/${slug}/generate?sim=1${Number.isFinite(simulationDelayMs) ? `&sim_delay_ms=${simulationDelayMs}` : ""}`
            : `/api/concepts/${slug}/generate`;
        await run(endpoint, {});
    };
                        ...
                        <>
                            <div className="space-y-2">
                                <div
                                    ...
                                    <span>{completedSteps.size}/{CONCEPT_STEPS.length} steps</span>
                                    <span className="font-mono">{toolCount} tool call{toolCount === 1 ? "" : "s"}</span>
                                </div>
                                <div className="flex gap-1.5">
                                    {CONCEPT_STEPS.map(({key, label}) => {
                                ...
                                <div className="h-1 rounded-full bg-outline-variant/30 overflow-hidden">
                                    <div
                                        ...
                                        style={{width: `${(completedSteps.size / CONCEPT_STEPS.length) * 100}%`}}
                                    />
                                </div>
                            </div>
                            <div
